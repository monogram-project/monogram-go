...
define classify_item( item, subsequent_items );
    ...
    if L == 1 then
        ...
        returnif( ch_first == subscrw(1, macro_mark) )( tt_force );
        returnif( inside_form and (ch_first == subscrw(1, colon)) )( tt_label );
        returnif( locchar( ch_first, 1, '({[' ) )( tt_open );
        returnif( locchar( ch_first, 1, ']})' ) )( tt_close );
        returnif( ch_first == `,` or ch_first == `;` )( tt_sep );
    endif;
    returnif( item.is_form_end )( tt_end );
    returnif( item.is_sign )( tt_sign );
    returnif( fast_lmember( item, inferred_form_starts ) )( tt_start );
        
    returnif( fast_lmember( item, inferred_forced_words ) )( tt_macro );

    lvars rest1 = subsequent_items;
    lvars next_item = not(rest1.null) and rest1.hd;

    returnunless( next_item )( tt_id );
        ...
        until pop11_try_nextreaditem( closing_keywords ) do
            if proglist.null then
                ...
                if first_expr_in_part and tokentype1 == "breaker" and unglue_option then
                    ...
                    tt_id -> tokentype1;
                elseif not(first_expr_in_part) and not(prev_expr_terminated) and tokentype1 == tt_label and unglue_option then
                    [^unglue_option ^^proglist] -> proglist;
                    unglue_option -> item1;
                    "breaker" -> tokentype1;
                endif;
                if tokentype1 == tt_sep or tokentype1 == tt_sign or tokentype1 == tt_close or tokentype1 == tt_label then
                    mishap( 'Unexpected item at start of expression (in ' >< opening_word >< ')', [^item1] )
                elseif tokentype1 == tt_end then
    ...
    until null( proglist ) do
        ...
        if p and p <= prec then
            ...
            elseif item1 == "." then
                ...
                if tokentype2 == tt_id or tokentype2 == "breaker" then
...
define read_opt_expr_prec( prec, accept_newline );
    ...
    returnif( tt == tt_sep or tt == tt_close or tt == tt_end or tt == "breaker" or tt == tt_label or tt == tt_sign )( false );
